        ...
        if is_directory:
            ...
            for filename in os.listdir(data_path):
                ...
                if filename.endswith(".pdf"):
                    loader = PyPDFLoader(file_path)              #PyPDFLoader -----------------
...
# Set up LLM and retrieval - using mistral as default model since it's smaller and more commonly available
local_model = "llama3.2"  
llm = ChatOllama(model=local_model, temperature=0.4)
...
def setup_system_chain():
    ...
    try:
        ...
        if not system_vector_db:
            print("No system documents found - system chain will not be available")
            system_retriever = None
            system_chain = None
            return False
...
def setup_user_chain(file_path):
    ...
    try:
        ...
        if not user_vector_db:
            print("Failed to process user document")
            return False
...
if not setup_system_chain():
    print("Warning: System chain initialization failed - no system documents available")
...
def chat_with_file():
    ...
    if not active_chain:
        return jsonify({
            ...
            "response": "I don't have any documents to reference. Please upload a PDF, PPT, or DOCX file first."
        }), 400
